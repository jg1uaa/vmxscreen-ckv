 ...
 *    Modified by SASANO Takayoshi at 2017/07/14.
 *
 *----------------------------------------------------------------------
 */
...
LOCAL	CONST	B	*OEMName = "T-Engine Video Device";
...
#if 0	// none uses these functions, disable them
/*
...
*/
EXPORT	ERR	mapFrameBuf(void *paddr, W len, void **laddr)
{
	UW	attr;
...
EXPORT	ERR	initSCREEN(void)
{
	...
	if (Vinf.framebuf_total > 0 &&
		Vinf.vramsz > Vinf.framebuf_total) return ER_OBJ;
...
*/
EXPORT	ERR	getSCRXSPEC(DEV_SPEC *spec, W mode)
{
	ER	er;
...
*/
EXPORT	ERR	getSCRSPEC(DEV_SPEC *spec)
{
	spec->planes = 1;
...
#define	putTC(x, y, z)	(x)[(y)] = (z)
	...
	if (desc != NULL) {
		for (; n < MODESTR_SIZE && *desc != TK_NULL; n++, desc++) {
			putTC(str, n, *desc);
		}
	}
...
*/
EXPORT	ERR	getsetSCRNO(W *scnum, BOOL suspend, BOOL set)
{
	if (set) return ER_NOSPT;	/* mode is not supported */
...
/*
        get / set color map
*/
EXPORT	WERR	getsetSCRCOLOR(COLOR *cmap, BOOL set)
{
	if (Vinf.cmapent <= 0) return ER_OBJ;	/* colormap is not used */
...
*/
EXPORT	ERR	getsetSCRBRIGHT(W *brightness, BOOL set)
{
	if (Vinf.fn_bright) return (*Vinf.fn_bright)(brightness, set);
	return ER_NOSPT;		/* not supported */
}
/*
        obtain the screen update function
*/
EXPORT	ERR	getSCRUPDFN(FP *updfn)
{
...
*/
EXPORT	ERR	setSCRUPDRECT(RECT *rp)
{
	if (! Vinf.fn_updscr) return ER_NOSPT;	/* not supported */
...
*/
EXPORT	ERR	getsetSCRVFREQ(W *vfreq, BOOL set)
{
	if ((Vinf.attr & SUPPORT_VFREQ) == 0) return ER_NOSPT; /* not supported */
...
*/
EXPORT	ERR	getSCRDEVINFO(ScrDevInfo *inf)
{
...
*/
EXPORT	ERR	setSCRWRITE(W kind, void *buf, W size)
{
